...
class Certbot
{
     ...
     *
     * Create or update configuration for a node, including letsencrypt domain
     * validation process, certificates (re)generation, node certificates update,
     * node deployment
     *
     * @param string $node    node name for which configuration is requested 
     *                        {@required true}
    ...
    function generateConfAndCerts($node, $contact, $domains)
    {
        ...
        $bashDomains=$bashDomains . "\"";
        file_put_contents(
            $OSALEInstallDir . "/data/$node.conf",
        ...
        $rc->domains=$domains;
        exec(
            "sudo -H " . $OSALEInstallDir . "/bin/generateCerts.sh " . $node,
            $out,
            $execRc
        );
        if ($execRc != 0 ) {
            ...
            throw new RestException(
                500,
                "Error while callng letsencrypt =>" .$leErr
            );
        }
        return $rc;
    }
     ...
     * Delete config
     *
     * Delete configuration for a node, including letsencrypt certificates 
     * revokation, node certificates cleanning
     *
     * @param string $node node name for which configuration is requested 
     *                     {@required true}
     *
    ...
    function deleteConf($node)
    {
        ...
        exec(
            "sudo -H " . $OSALEInstallDir . "/bin/revokeCerts.sh " . $node,
            $out,
            $execRc
        );
        if ($execRc != 0 ) {
